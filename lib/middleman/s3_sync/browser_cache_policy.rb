module Middleman
  module S3Sync
    class BrowserCachePolicy
      ...
      def BrowserCachePolicy.add_caching_policy(content_type, policy = {})
        caching_policies[content_type.to_s] = BrowserCachePolicy.new(policy)
      end

      def BrowserCachePolicy.caching_policies
        @caching_policies ||= Map.new
      end

      def BrowserCachePolicy.caching_policy_for(content_type)
